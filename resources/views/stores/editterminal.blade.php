...
@section('content')
    <div class="card shadow-sm">
        <div class="card-header">
            <h3 class="card-title">Edit Existing Terminal</h3>
            ...
            <form name="frm_save_terminal" id="FORM_SAVE_TERMINAL">
                <div class="form-body">
                     <span id="hidden_fields">
                        {!! csrf_field() !!}
                         <input type="hidden" name="pt_id" value="{{ $termianl_info->pt_id }}" />
                    </span>
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="text" name="pt_terminal_name" id="PT_TERMINAL_NAME" class="form-control" required="required" maxlength="255"  value="{{ $termianl_info->pt_terminal_name }}" />
                            </div>
                        </div>
                        <div class="col-md-4">
                            ...
                            <select class="form-select form-control" data-control="select2" id="PT_STORE_ID" name="pt_store_id">
                                ...
                                @foreach($lst_stores as $index => $store_info)
                                    <option {{ $termianl_info->pt_store_id == $store_info->ps_id ? "selected" : "" }} value="{{ $store_info->ps_id }}">{{ $store_info->ps_store_name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-4">
                            ...
                            <select class="form-select form-control" data-control="select2" id="PT_MANAGER_ID" name="pt_manager_id">
                                ...
                                @foreach($lst_managers as $index => $manager_info)
                                    <option {{ $termianl_info->pt_manager_id == $manager_info->id ? "selected" : "" }} value="{{ $manager_info->id }}">{{ $manager_info->u_fullname }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <select class="form-select form-control" data-control="select2"  id="PT_WAREHOUSES_ID" name="pt_warehouse_id">
                                    @foreach($lst_warehouses as $index => $warehouse_info)
                                        <option {{ $termianl_info->pt_warehouse_id == $warehouse_info->w_id ? "selected" : "" }} value="{{ $warehouse_info->w_id }}">{{ $warehouse_info->w_warehouse_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <label class="form-check form-switch form-check-custom form-check-solid">
                                    <input class="form-check-input" type="checkbox" {{ $termianl_info->pt_is_active == 1 ? "checked" : "" }} name="pt_is_active" id="PT_IS_ACTIVE"  value="1"  />
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <textarea style="width:100%;height:250px;resize:none" id="PT_DESCRIPTION"  class="form-control" name="pt_description"  cols="">{{ $termianl_info->pt_description }}</textarea>
                            </div>
                        </div>
                    </div>
