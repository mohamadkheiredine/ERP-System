...
<div class="card shadow-sm">
    <div class="card-header">
        <h3 class="card-title">Edit Case Status</h3>
    ...
    <form name="frm_save_status" id="FORM_SAVE_STATUS">
                <div class="form-body">
                     <span id="hidden_fields"> 
                        {!! csrf_field() !!} 
                        <input type="hidden" name="cc_id" value="{{ $status_info->cc_id }}" />
                     </span>
                    ...
                    <div class="row">
                              ...
                              <div class="form-group">
                                    <label class="control-label"> Status </label>
                                     <select name="fk_parent_status" id="FK_PARENT_STATUS"   class="form-control form-select" data-control="select2" data-placeholder="Select Parent Status">
                                            ...
                                            @foreach($lst_statuses as $key => $status)
                                                    <option {{ $status_info->fk_parent_status == $status->cc_id ? "selected" : "" }} value="{{ $status->cc_id }}">{{ $status->cc_status_title }}</option>
                                            @endforeach
                                    </select>
                                </div>
                        </div>
                        ...
                        <div class="col-md-4">
                             <div class="form-group">
                                ...
                                <input type="text" name="cc_status_title" id="CC_STATUS_TITLE" class="form-control" value="{{ $status_info->cc_status_title }}" />
                             </div>
                        </div>
                         <div class="col-md-4">
                             <div class="form-group">
                                <label class="control-label"> Status Color <span class="required"> * </span></label><br/>
                                <input type="color" name="cc_status_color" required="required" id="CC_STATUS_COLOR" class="form-control" value="{{ $status_info->cc_status_color }}" />
                             </div>
                        </div>  
                        <div class="col-md-12">
                             <div class="form-group">
                                <label class="control-label"> Description</label><br/>
                                <textarea style="width:100%;height:250px;resize:none" id="CC_STATUS_DESCRIPTION"  class="form-control" name="cc_status_description"  cols="">{{ $status_info->cc_status_description }}</textarea>
                             </div>
                        </div>
                    </div>
