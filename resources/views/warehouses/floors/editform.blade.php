            ...
            <form name="frm_save_floors" id="FORM_SAVE_FLOORS">
                <div class="form-body">
                     <span id="hidden_fields">
                       {!! csrf_field() !!}
                         <input type="hidden" name="wf_id" value="{{ $floor_info->wf_id  }}" />
                    </span>
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="text" name="wf_floor_title" id="WF_FLOOR_TITLE" class="form-control" required="required" maxlength="255"  value="{{ $floor_info->wf_floor_title  }}" />
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <select  name="fk_warehouse_id" id="FK_WAREHOUSE_ID" class="form-control form-select" data-control="select2" data-placeholder="Select Warehouse">
                                    ...
                                    @foreach ( $lst_warehouses as $key => $warehouse_info )
                                        <option {{ $floor_info->fk_warehouse_id == $warehouse_info->w_id ? "selected" : "" }} value="{{  $warehouse_info->w_id }}">{{  $warehouse_info->w_warehouse_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="form-group">
                                ...
                                <select  name="fk_zone_id" id="FK_ZONE_ID" class="form-control form-select" data-control="select2" data-placeholder="Select Zone">
                                    ...
                                    @foreach ( $lst_zones as $key => $zone_info )
                                        <option {{ $floor_info->fk_zone_id == $zone_info->wz_id ? "selected" : "" }}  value="{{  $zone_info->wz_id }}">{{  $zone_info->wz_zone_label }}</option>
                                    @endforeach
                                </select>
                                </div>
                            </div>
