...
@extends('layouts.layout',['page_title' => "Edit Cost cener"])
		...
		<form name="frm_save_costcenter" id="FORM_SAVE_COSTCENTER">
			<div class="form-body">
				<span id="hidden_fields"> 
                                    {!! csrf_field() !!} 
                                    <input type="hidden" name="ac_id" value="{{ $costcenter_info->ac_id  }}" />
                                </span>
				<div class="alert alert-success" style="display: none">
					<strong>Success!</strong> Cost Center Information is saved successfully!
				</div>
				...
				<div class="row">
					<div class="col-md-4">
						<div class="form-group">
							...
							<input type="text" name="ac_cost_center_label" id="AC_COST_CENTER_LABEL" class="form-control" required="required" maxlength="255" value="{{ $costcenter_info->ac_cost_center_label  }}" />
						</div>
					</div>
					<div class="col-md-4">
                                            <div class="form-group">
                                               <label class="control-label">Cost center Type</label>
                                                     <select  name="ac_type_id" id="AC_TYPE_ID" class="form-select" data-control="select2" data-placeholder="Select Costcenter Type">
                                                       ...
                                                       @foreach ( $lst_types as $key => $type_info )
                                                               <option {{ $costcenter_info->ac_type_id == $type_info->at_id ? "selected" : ""  }} value="{{ $type_info->at_id }}">{{ $type_info->at_type_name }}</option>
                                                       @endforeach
                                               </select>
                                           </div>
                                       </div>
                                               ...
                                               <label class="control-label">Cost center Category</label>
                                                     <select  name="ac_category_id" id="AC_CATEGORY_ID" class="form-select" data-control="select2" data-placeholder="Select Category">
                                                       ...
                                                       @foreach ( $lst_categories as $key => $category_info )
                                                               <option {{ $costcenter_info->ac_category_id ==  $category_info->cca_id ? "selected" : ""  }} value="{{ $category_info->cca_id }}">{{ $category_info->cca_category_name }}</option>
                                                       @endforeach
                                               </select>
                                           </div>
                                       </div>
                                               ...
                                               <label class="control-label">Cost center Status</label>
                                                     <select  name="ac_status_id" id="AC_STATUS_ID" class="form-select" data-control="select2" data-placeholder="Select Status">
                                                       ...
                                                       @foreach ( $lst_statuses as $key => $status_info )
                                                               <option {{ $costcenter_info->ac_status_id ==   $status_info->cs_id ? "selected" : ""  }} value="{{ $status_info->cs_id }}">{{ $status_info->cs_status_name }}</option>
                                                       @endforeach
                                               </select>
                                           </div>
                                       </div>
                                               ...
                                               <label class="control-label">Cost center Manager</label>
                                                     <select  name="ac_manager_id" id="AC_MANAGER_ID" class="form-select" data-control="select2" data-placeholder="Select Manager">
                                                       ...
                                                       @foreach ( $lst_managers as $key => $manager_info )
                                                               <option {{ $costcenter_info->ac_manager_id == $manager_info->id ? "selected" : ""  }} value="{{ $manager_info->id }}">{{ $manager_info->u_fullname }}</option>
                                                       @endforeach
                                               </select>
                                           </div>
                                       </div>
                                               ...
                                               <label class="control-label">Cost center Parent</label>
                                                     <select  name="ac_parent_cost_center_id" id="AC_PARENT_COST_CENTER_ID" class="form-select" data-control="select2" data-placeholder="Select Costcenter">
                                                       ...
                                                       @foreach ( $lst_costcenters as $key => $costcenter_info )
                                                               <option {{ $costcenter_info->ac_parent_cost_center_id == $costcenter_info->ac_id ? "selected" : ""  }} value="{{ $costcenter_info->ac_id }}">{{ $costcenter_info->ac_cost_center_label }}</option>
                                                       @endforeach
                                               </select>
                                           </div>
                                       </div>
					<div class="col-md-12">
						...
						<textarea style="width:100%;height:250px;" name="ac_cost_center_description" id="AC_COST_CENTER_DESCRIPTION" class="form-control">{{ $costcenter_info->ac_cost_center_description }}</textarea>
					</div>
				</div>
