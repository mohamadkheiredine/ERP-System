...
<td>
	 <select class="bs-select form-control row-select" name="tm_ledger_account" id="" data-actions-box="true">
            <option value="0"> --Select Account--</option>
            @foreach ( $lst_accounts as $key => $account_info )
                    <option {{ $movement_info->tm_ledger_account == $account_info->aa_id ? "selected" : "" }} value="{{ $account_info->aa_id }}">{{ $account_info->aa_account_ref . " - " . $account_info->aa_account_label }}</option>
            @endforeach
    </select>
</td>
<td>
