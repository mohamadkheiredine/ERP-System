...
Page Description :
Page of edit exchange rate to save it into the datbase
***********************************************************/
             ...
             <form name="frm_save_rate" id="FORM_SAVE_RATE">
                <div class="form-body">
                     <span id="hidden_fields">
                      {!! csrf_field() !!}
                      <input type="hidden" name="er_id" value="{{ $exchange_rate->er_id }}" />
                    </span>
                    ...
                    <div class="row">
                                    ...
                                    <label class="control-label"> Source Currency <span class="required"> * </span></label>
                                     <select class="bs-select form-control" name="er_from_currency" id="ER_FROM_CURRENCY" data-actions-box="true">
                                            ...
                                            @foreach ( $lst_currencies as $key => $currency_info )
                                                    <option {{ $exchange_rate->er_from_currency == $currency_info->cc_id ? "selected" : "" }} value="{{ $currency_info->cc_id }}">{{ $currency_info->cc_currency_code . " - " . $currency_info->cc_currency_name  }}</option>
                                            @endforeach
                                    </select>
                                </div>
                        </div> 
                              ...
                              <div class="form-group">
                                    <label class="control-label"> Destination Currency <span class="required"> * </span></label>
                                     <select class="bs-select form-control" name="er_to_currency" id="ER_TO_CURRENCY" data-actions-box="true">
                                            <option value="">-- Select Currency --</option>
                                            @foreach ( $lst_currencies as $key => $currency_info )
                                                    <option {{ $exchange_rate->er_to_currency == $currency_info->cc_id ? "selected" : "" }} value="{{ $currency_info->cc_id }}">{{ $currency_info->cc_currency_code . " - " . $currency_info->cc_currency_name  }}</option>
                                            @endforeach
                                    </select>
                                </div>
                        </div>
                        <div class="col-md-4">
                              <div class="form-group">
                                    ...
                                    <input type="text" name="er_exchange_rate" id="ER_EXCHANGE_RATE" class="form-control" required="required" maxlength="255"  value="{{ $exchange_rate->er_exchange_rate }}" />
                                </div>
                        </div>
                    </div>
