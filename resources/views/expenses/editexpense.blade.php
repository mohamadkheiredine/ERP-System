...
@extends('layouts.layout',['page_title' => "Expenses - Edit Expense Record"])
            ...
            <form name="frm_save_expense" id="FORM_SAVE_EXPENSE">
                <div class="form-body">
                    <span id="hidden_fields">
                        {!! csrf_field() !!}
                        <input type="hidden" name="ac_id" value="{{ $expenses_info->ac_id }}" />
                    </span>
                    ...
                    <div class="row">
                            ...
                            <div class="form-group">
                                ...
                                <select  name="ac_category_id" id="AC_CATEGORY_ID" class="form-control form-select" data-control="select2" data-placeholder="Select Category" >
                                    ...
                                    @foreach ( $lst_categories as $key => $category_info )
                                        <option {{ $expenses_info->ac_category_id == $category_info->ec_id ? "selected" : "" }} value="{{ $category_info->ec_id }}">{{ $category_info->ec_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="form-group">
                                ...
                                <select  name="ac_status_id" id="AC_STATUS_ID" class="form-control form-select" data-control="select2" data-placeholder="Select Status" >
                                    ...
                                    @foreach ( $lst_expenses_status as $key => $status_info )
                                        <option {{ $expenses_info->ac_status_id == $status_info->ss_id  ? "selected" : "" }} value="{{ $status_info->ss_id }}">{{ $status_info->ss_status_title }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="form-group">
                                ...
                                <select  name="ac_employee_id" id="AC_EMPLOYEE_ID" class="form-control form-select" data-control="select2" data-placeholder="Select Employee" >
                                    ...
                                    @foreach ( $lst_employees as $key => $employee_info )
                                        <option {{ $expenses_info->ac_employee_id == $employee_info->id  ? "selected" : "" }} value="{{ $employee_info->id }}">{{ $employee_info->u_fullname }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="text" name="ac_expense_date" id="AC_EXPENSE_DATE" class="form-control" required="required" maxlength="15" value="{{ $expenses_info->ac_expense_date }}" />
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label class="control-label">Amount <span class="required"> * </span></label>
                                <input type="text" name="ac_amount" id="AC_AMOUNT" class="form-control" required="required" maxlength="15" value="{{ $expenses_info->ac_amount }}" />
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <select  name="ac_currency_id" id="AC_CURRENCY_ID" class="form-control form-select" data-control="select2" data-placeholder="Select Currency">
                                    ...
                                    @foreach ( $lst_currencies as $key => $currency_info )
                                        <option {{ $expenses_info->ac_currency_id == $currency_info->cc_id  ? "selected" : "" }} value="{{ $currency_info->cc_id }}">{{ $currency_info->cc_currency_name }}&nbsp;(&nbsp;{{ $currency_info->cc_currency_code }}&nbsp;)</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="form-group">
                                ...
                                <select  name="ac_payment_type" id="AC_PAYMENT_TYPE" class="form-control form-select" data-control="select2" data-placeholder="Select Payment Type">
                                    ...
                                    @foreach ( $lst_payment_types as $key => $type_info )
                                        <option {{ $expenses_info->ac_payment_type == $type_info->pt_id  ? "selected" : "" }}  value="{{ $type_info->pt_id }}">{{ $type_info->pt_payment_type }}&nbsp;)</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
