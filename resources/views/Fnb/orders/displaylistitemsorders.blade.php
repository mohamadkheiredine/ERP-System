@foreach($lst_menu_items_orders as $index => $item_order_info)
    <tr class="odd gradeX" data-oi_id="{{ $item_order_info->oi_id }}">
        ...
        <td>
            <a href="#" data-oi_id="{{ $item_order_info->oi_id }}"
                id="OPEN_MODIFIERS_ITEM_ORDER_{{ $item_order_info->oi_id }}" data-bs-toggle="modal"
                data-bs-target="#ModelPopUpModifiers">
        ...
        <td>{{ number_format($item_order_info->oi_total_price, 2) }}&nbsp;&nbsp;<b>{{ $item_order_info->Currency ? $item_order_info->Currency->cc_currency_code : "" }}</b>
        </td>
    </tr>
@endforeach
