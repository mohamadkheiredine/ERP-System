...
@foreach($lst_salarydetails  as $index => $details_info)
    <tr  class="odd gradeX" data-pd_id="{{ $details_info->pd_id }}">
        ...
        <td>{{ $details_info->Company ? $details_info->Company->cd_company_name : "-" }}</td>
        <td>{{ $details_info->Employee->u_fullname }}</td>
        <td>{{ $details_info->pd_basic_salary }} <b>{{ $details_info->Currency ? $details_info->Currency->cc_currency_code : "" }}</b></td>
        <td>{{ $details_info->pd_total_comissions }} <b>{{ $details_info->Currency ? $details_info->Currency->cc_currency_code : "" }}</b></td>
        <td>{{ $details_info->pd_deductions }}&nbsp;<b>{{ $details_info->Currency ? $details_info->Currency->cc_currency_code : "" }}</b></td>
        <td>{{ $details_info->pd_allowances }}&nbsp;<b>{{ $details_info->Currency ? $details_info->Currency->cc_currency_code : "" }}</b></td>
