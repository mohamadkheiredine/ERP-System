             ...
             <form name="frm_save_status" id="FORM_SAVE_STATUS">
                <div class="form-body">
                     <span id="hidden_fields">
                       {!! csrf_field() !!}
                       <input type="hidden" id="PS_ID" name="ps_id" value="{{ $status_info->ps_id }}" /> 
                    </span>
                        ...
                        <div class="col-md-4">
                             <div class="form-group">
                                <label class="control-label">Parent Status</label>
                                 <select class="bs-select form-control" name="ps_parent_status" id="PS_PARENT_STATUS" data-actions-box="true">
                                        ...
                                        @foreach ( $lst_plan_status as $key => $sta_info )
                                                <option {{ $status_info->ps_parent_status == $sta_info->ps_id ? "selected='selected'" : "" }} value="{{ $sta_info->ps_id }}">{{ $sta_info->ps_status_title }}</option>
                                        @endforeach
                                </select>
                            </div>
                        </div>
                         <div class="col-md-12">
                             <div class="form-group">
								...
								<textarea name="ps_status_description" id="PS_STATUS_DESCRIPTION" class="form-control" style="width:100%;height:250px;">{{ $status_info->ps_status_description }}</textarea>
                            </div>
                        </div>
                    </div>
