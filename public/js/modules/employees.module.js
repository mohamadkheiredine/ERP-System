...
var employees_module = {
			DisplayListEmployees : function(){
			    ...
			    $.ajax
		        ({
		            url : base_url + "/payroll/employees/displaylist",
		            data : params,
		            dataType : "json",
		            type : "GET",
		            success : function(response){
		            	$('#LstEmployees').html(response.display);
		            	   $.pagination = $('#EmployeesPagination').twbsPagination({
			...
			EditEmployeeInfo : function(){
			    ...
			    var base_url 	= $('input[name=base_url]').val();
			    window.location.href = base_url + "/payrolls/employees/editform/" + user_id;
			},
			DeleteEmployeeInfo : function(){
				 ...
				 bootbox.confirm("Are you sure you want to delete ?", function(result){
						...
						if(result == true)
						{
							...
							$.ajax
							({
								url : base_url + "/payroll/employees/deleteemployee",
								data : str_params,
								dataType : "Json",
								type : "DELETE",
								success : function(response){
			...
			SaveEmployeeInfo : function(){
				return employees_module.SaveEmployeeInfoSubmitHandler();
			},
    SaveEmployeeInfoSubmitHandler : function(){
				 var ProfileForm = $('#FORM_SAVE_EMPLOYEES');
		         ...
		         ProfileForm.validate({
		             ...
		             submitHandler: function (form) {
		                ...
		                var FormDataFields = $("#FORM_SAVE_EMPLOYEES");
		                ...
		                $.ajax
                        ({
                            url: base_url + '/payroll/employees/saveinfo',
                            data: data,
                            async: false,
                            cache: false,
                            method: 'post',
                            ...
                            success: function (response) {
                                if (response.is_error == 0) {
                                	 window.location.href = base_url + "/payrolls/employees";
                                } else {
