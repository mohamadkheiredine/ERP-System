...
LIMIT 10;';
    ...
    {
        $data = array( );
        return Response()->view("dashboard.inventory",$data);
    }
    ...
    public function GetStockByProducts(Request $request)
    {
        ...
        foreach ($total_stock_products as $key => $product_info ) {
            $stock_products[] = array(
                'product' =>   $product_info->p_product_name,
                'value' =>   $product_info->stock_quantity
            );
        }

        $result_array['is_error'] = 0;
        $result_array['error_msg'] = "";
        $result_array['stock_products'] = $stock_products;
    ...
    public function GetStockByWarehouse(Request $request)
    {
        ...
        foreach ($total_stock_warehouse as $key => $warhouse_info ) {
            $stock_warehouse[] = array(
                'warehouse' =>   $warhouse_info->w_warehouse_name,
                'value' =>   $warhouse_info->total_stock
            );
        }

        $result_array['is_error'] = 0;
        $result_array['error_msg'] = "";
        $result_array['stock_warehouse'] = $stock_warehouse;
    ...
    {
        $query = "SELECT ii_item_id , SUM(ii_item_qyt) as total_sel_quantity , p_product_name FROM retailerp_db.billing_invoice_items left join inventory_products ON p_id = ii_item_id  group by ii_item_id;";


        $total_selling_products = DB::select($query);
