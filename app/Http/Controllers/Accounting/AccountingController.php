...
class AccountingController extends Controller
{
    ...
    public function Displaylistaccounttotals(Request $request)
    {
        ...
        $lst_total_amounts_by_currencies = array();

        foreach ($lst_accounts as $row) {
            $currency = $row->cc_currency_code;

            if (!isset($lst_total_amounts_by_currencies[$currency])) {
                $lst_total_amounts_by_currencies[$currency] = 0;
            }

            $lst_total_amounts_by_currencies[$currency] += ($row->total_debit - $row->total_credit);
        }

        $data = array(
            "lst_accounts" => $lst_accounts,
            "lst_total_amounts_by_currencies" => $lst_total_amounts_by_currencies,
        );
