...
class AccountingController extends Controller
{
    ...
    public function downloadStatementToExcel()
    {
        $lst_accounts = DB::select("SELECT
            curr.cc_id,
            tm.tm_sub_ledger_account,
            curr.cc_currency_code,
            accounts.aa_account_ref,
            accounts.aa_account_label,
            SUM(tm.tm_debit) AS total_debit,
            SUM(tm.tm_credit) AS total_credit,
            SUM(tm.tm_debit) - SUM(tm.tm_credit) AS total_balance
            FROM acc_transaction_movements tm
            LEFT JOIN acc_accounting_accounts accounts
        ...
        $collection = collect($lst_accounts);

        // 3️⃣ Compute totals per currency
        $totalsByCurrency = $collection->groupBy('cc_currency_code')
            ->map(fn($group) => [
        ...
        $data = [
            ['Currency', 'Account Label', 'Account Code', 'Debit', 'Credit', 'Total Balance']
        ];

        foreach ($lst_accounts as $account) {
            $data[] = [
                $account->cc_currency_code,
                $account->aa_account_label,
                $account->aa_account_ref,
        ...
        $csv->insertAll($data);
        $csv->output('account-statement.csv');
    }
