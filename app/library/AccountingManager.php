...
class AccountingManager
{
     ...
     */
    public function GenerateReceiptCode($params_array = [])
    {
        $fyear = date("Y");

        // FIX: get company_id from params, not session
        $company_id = $params_array['company_id'] ?? session('company_id');

        $company_info = Companies::find($company_id);

        if (!$company_info) {
            throw new \Exception("Company not found in GenerateReceiptCode. ID = " . $company_id);
        }
